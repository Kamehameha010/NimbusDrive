...
locals {
  api_resources = merge(
    { for k, v in aws_api_gateway_resource.root : k => v.id },
    { for k, v in aws_api_gateway_resource.child : k => v.id },
  )
}

resource "aws_api_gateway_resource" "root" {
  for_each    = { for k, v in var.api_gateway_resources : k => v if v.parent_key == null }
  rest_api_id = aws_api_gateway_rest_api.this.id
...
resource "aws_api_gateway_resource" "child" {
  for_each    = { for k, v in var.api_gateway_resources : k => v if v.parent_key != null }
  rest_api_id = aws_api_gateway_rest_api.this.id
  parent_id   = aws_api_gateway_resource.root[each.value.parent_key].id
  path_part   = each.value.path_part
}
...
resource "aws_api_gateway_integration" "this" {
  ...
  connection_id           = each.value.connection_type == "VPC_LINK" ? try(each.value.vpc_link_id, null) : null
